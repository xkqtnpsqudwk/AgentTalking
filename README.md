...
inject_initial_relationship(
    ...
    relationship_name="헤어진 연인",
    emotional_context="예전에 깊이 가까웠지만 지금은 어색함이 남아 있다."
)
```
...
그리고 `inject_public_fact()`로 각 Agent가 전체 관계 구도를 어느 정도 알고 시작하도록 했다.  
그래서 전 연인, 현재 연인, 서로의 관계를 의식하는 대화가 나오기 쉬워졌다.
...
관계가 단순 문자열로만 있으면 대화 변화가 약해서 `emotional_context`와 `relationship_facts`를 추가했다.
...
inject_initial_relationship(
    ...
    relationship_name="헤어진 연인",
    emotional_context=(
        "예전에 깊이 가까웠지만 지금은 어색함이 남아 있다. "
        "서로를 신경 쓰지만 먼저 티 내지는 않는다."
    )
)
```

현재 관계 정보는 일반 Memory와 별도로 유지된다.

```text
Memory              → 최근 몇 개만 대화에 사용
relationship_facts  → 연애중, 헤어진 연인, 반말, 배려, 어색함 같은 관계 정보는 항상 사용
relationship_states → 현재 연인인지, 전 연인인지 구조화해서 전달
```

그래서 `MAX_MEMORY_PER_TARGET=3`이어도 연애중/전 연인 정보가 프롬프트에서 빠지지 않는다.

대화 방향은 다음과 같다.

```text
topic은 커피, 식사, 공부, 날씨, 근황 같은 일상 주제로 유지한다.
관계 정보는 topic을 바꾸지 않고 말투, 거리감, 반응, 배려, 어색함에만 영향을 준다.
```
...
헤어진 연인 포함 → 오랜만에 마주친 어색함
연애중 관계 포함 → 서로의 최근 컨디션
복잡한 관계 포함 → 과거와 현재 관계 사이에서 느끼는 어색함
```
